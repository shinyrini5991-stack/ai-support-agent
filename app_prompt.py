...
def llm_tool(query, memory_context="", mode="basic"):
...
# ----------------PROMPT Comparison-----------------#
st.sidebar.header("🧠 Prompt Comparison")

if st.sidebar.button("Run Prompt Comparison"):
    ...
    for q in test_queries:
        ...
        # BASIC
        prompt_mode = "basic"
        basic_resp = llm_tool(q)

        # STRICT
        prompt_mode = "strict"
        strict_resp = llm_tool(q)

        results.append({
            "Query": q,
            "Basic Output": basic_resp,
            "Strict Output": strict_resp
        })
